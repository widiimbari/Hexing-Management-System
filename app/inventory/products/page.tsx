...
import { Search, FileSpreadsheet, X, ArrowRight, Calendar as CalendarIcon, Boxes } from "lucide-react";
...
export default function ProductsPage() {
  ...
  const fetchStats = useCallback(async () => {
      if (!currentFilters.startDate && !currentFilters.endDate && currentFilters.period !== "all") return;
  ...
  const handlePeriodChange = (value: string) => {
    setPeriod(value);
    setStartSerial(""); // Clear serial search to enable Output Mode
    setEndSerial("");
  ...
  const handleApplyFilters = () => {
      ...
      // Set loading state immediately based on target mode
      if (startSerial) {
          setLoading(true);
  ...
  useEffect(() => {
      if (hasSearched) {
          // Priority: If Serial is present -> Always Product List Mode
          if (currentFilters.startSerial) {
              fetchData();
          }
          // Else if Date Range is present OR Period is 'All Time' -> Output Mode
          else if ((currentFilters.startDate && currentFilters.endDate) || currentFilters.period === "all") {
              fetchStats();
          } 
  ...
  // Determine View Mode
  const isOutputMode = (!!(currentFilters.startDate && currentFilters.endDate) || (currentFilters.period === "all" && hasSearched)) && !currentFilters.startSerial;
