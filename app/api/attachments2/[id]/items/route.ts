...
export async function GET(
  ...
  try {
    ...
    if (view === "box") {
      ...
      // Filter logic manual if search exists
    ...
    if (view === "pallet") {
        ...
        const productBoxes = await db.product.findMany({
            where: { attachment2_id: attachmentId, box_id: { not: null } },
            select: { box_id: true }
        });
        ...
        const boxPalletMap = new Map<number, number>(); 
        ...
        const productCounts = await db.product.groupBy({
            by: ['box_id'],
            where: { attachment2_id: attachmentId, box_id: { in: boxIds } },
            _count: { id: true }
        });
    ...
    console.error("[GET_SLAVE_ITEMS]", error);
